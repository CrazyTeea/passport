<template>
  <div>
    <b-button :variant="(isNav === true) ? 'none' : 'secondary'" @click="showModal('export-data')"
              class="mb-2 float-left">Выгрузка данных
      <slot></slot>
    </b-button>
    ...
    <b-modal size="xl" centered ref="export-data">
      <template v-slot:modal-header>
        Выгрузить данные
        <b-button @click="closeModal('export-data')" size="sm" variant="outline-danger"><i class="fas fa-times"></i>
        </b-button>
      </template>
      ...
      <div class="container">
        ...
        <b-button v-if="exportFilter.length === 8 && checkUser(['admin','root'])" @click="exportFilter = []" variant="outline-warning">Снять выделение
        </b-button>
        <b-button v-if="exportFilter.length < 8 && checkUser(['admin','root'])"  @click="exportFilter = options.map(item=>item.value)" variant="outline-success">Выделить все
        </b-button>
    ...
    <b-modal size="xl" centered ref="export-stat">
      <template v-slot:modal-header>
        Выгрузить данные
        <b-button @click="closeModal('export-stat')" size="sm" variant="outline-danger"><i class="fas fa-times"></i>
        </b-button>
      </template>
      <template v-slot:modal-footer>
        <div></div>
...
<script>
import {BButton, BFormCheckbox,BAlert,
  BFormGroup, BFormInput, BModal} from 'bootstrap-vue'
...
export default {
  ...
  components: {
    BButton, BModal, BFormInput,BAlert,
